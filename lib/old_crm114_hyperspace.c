					     ...
					     long featurecount)
{
  ...
  if ((*db)->cb.class[whichclass].features
    ...
    {
      int old_db_size, db_increase, new_db_size;
      int move_old_begin_index, move_new_begin_index;
	...
	fprintf (stderr, "Sorry, insufficient space in db.  Must expand\n");
      ...
      db_increase = (*db)->cb.block[whichclass].allocated_size
	+ featurecount * (int)sizeof (HYPERSPACE_FEATUREBUCKET);
      ...
      if (crm114__internal_trace)
	fprintf (stderr, "just before realloc\n");
	...
	return (CRM114_NOMEM);
      *db = newdb;
      ...
      //   move the remaining blocks up to give the space to whichclass's
      //   block.
      if (whichclass < (*db)->cb.how_many_blocks)
	{
	  move_old_begin_index =(*db)->cb.block[whichclass+1].start_offset;
	  move_new_begin_index = move_old_begin_index + db_increase;
	  bytes_to_move = 0;
	  for (i = whichclass+1; i < (*db)->cb.how_many_blocks; i++)
	    {
	      if (crm114__internal_trace)
	      fprintf (stderr, "block %ld is %zu bytes\n", i,
